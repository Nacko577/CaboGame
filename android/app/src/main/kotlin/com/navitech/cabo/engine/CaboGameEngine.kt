package com.navitech.cabo.engine
...
class CaboGameEngine {
    ...
    fun startGame() {
        ...
        val firstDiscard = deck.removeAt(deck.lastIndex)
    ...
    fun attemptMatchDiscard(playerID: String, handIndex: Int): Boolean {
        ...
        if (state.phase == TurnPhase.INITIAL_PEEK) throw GameRuleError("This action is not valid right now.")
        state = state.copy(kingSwapHighlight = null)
    ...
    fun resolveSpecialAction(playerID: String, action: SpecialAction) {
        ...
        when {
            ...
            top.rank == Rank.KING && action is SpecialAction.SwapCards -> {
                ...
                state = state.copy(
                    players = newPlayers,
                    kingSwapHighlight = KingSwapHighlight(
                        fromPlayerID = action.fromPlayerID,
                        fromHandIndex = action.fromCardIndex,
                        toPlayerID = action.toPlayerID,
                        toHandIndex = action.toCardIndex
                    )
                )
            }
    ...
    fun callCabo(playerID: String) {
        ...
        state = state.copy(caboCallerID = playerID, kingSwapHighlight = null)
        endTurn()
    }
